...
import { collection, addDoc, getDocs, deleteDoc, updateDoc, doc, onSnapshot, setDoc } from 'firebase/firestore';
...
const TiptapEdit = ({ content, setDescription }) => {
    const editor = useEditor({
        extensions: [StarterKit, Underline, Link],
        content: content,
        onUpdate: ({ editor }) => {
...
export default function Admin() {
    ...
    const [blogEditObject, setBlogEditObject] = useState();
    const [blogEditTitle, setBlogEditTitle] = useState('');
    const [blogEditSlug, setBlogEditSlug] = useState('');
    const [blogEditDescription, setBlogEditDescription] = useState('');
    const [blogEditContent, setBlogEditContent] = useState('');
    const [blogEditDate, setBlogEditDate] = useState(new Date().toISOString().split('T')[0]);
    const [blogEditName, setBlogEditName] = useState('');
    const [blogEditImageUpload, setBlogEditImageUpload] = useState(null);
    const [blogEditError, setBlogEditError] = useState('');
    ...
    const BlogSubmit = async (isDraft = false) => {
        if (!BlogImageUpload || !BlogTitle || !BlogDescription || !BlogSubDescription || !BlogDate || !BlogName || !BlogSlug) {
    ...
    const toggleEdit = (blog = '', value) => {
        ...
        if (blog === '') {
            ...
            setBlogEdit(value)
            setBlogEditObject(blog)
            setBlogEditTitle(blog.title)
            setBlogEditSlug(blog.slug || '')
            setBlogEditDescription(blog.description)
            setBlogEditContent(blog.subDescription)
            setBlogEditDate(new Date(blog.date).toISOString().split('T')[0])
            setBlogEditName(blog.name)

        }

    }

    // ? Handle Edit Blog Submit
    const BlogEditSubmit = async () => {
        ...
        let imageRef, url = blogEditObject.imageURL
        try {

            if (blogEditImageUpload) {

                imageRef = ref(storage, `blogs/${blogEditImageUpload?.name + uuidv4()}`);

                const snapshot = await uploadBytes(imageRef, blogEditImageUpload);
            ...
            const blogRef = doc(db, "blogs", blogEditObject.id)

            const blogPost = {
                name: blogEditName,
                title: blogEditTitle,
                description: blogEditDescription,
                subDescription: blogEditContent,
                imageURL: url,
                date: blogEditDate,
                slug: blogEditSlug
            };

            await setDoc(blogRef, blogPost, { merge: true });
    ...
    const EditBlog = () => {
        ...
        return (
            <div>
                <div className='text-2xl p-5'>Edit Blog</div>
                <div className='px-5'>
                    ...
                    <div className="form-group">
                        ...
                        <input
                            type="text"
                            value={blogEditDescription}
                            onChange={(e) => setBlogEditDescription(e.target.value)}
                            className="form-input"
                        />
                    </div>
                    <div className="form-group">
                        <label className="form-label">Content</label>
                        <TiptapEdit content={blogEditObject.subDescription} setDescription={setBlogEditContent} />
                    </div>
                    <div className="form-group">
                        <label className="form-label">Preview</label>
                        <div className="box">{parse(blogEditContent)}</div>
                    </div>
                    <div className="form-group">
                        ...
                        <input
                            type="date"
                            value={blogEditDate}
                            onChange={(e) => setBlogEditDate(e.target.value)}
                            className="form-input"
                        />
                    </div>
                    <div className="form-group">
                        ...
                        <input
                            type="text"
                            value={blogEditName}
                            onChange={setBlogEditName}
                            className="form-input"
                        />
                        {blogEditError && <p className="error-message">{blogEditError}</p>}
                    </div>

                    <div className="form-group">
                        <label className="form-label">Current Image</label>
                        <img
                            src={blogEditObject.imageURL}
                            width={500}
                        />
                    </div>
                    ...
                    <div className="form-group">
                        ...
                        <input
                            ...
                            type="file"
                            style={{ display: "none" }}
                            onChange={(e) => setBlogEditImageUpload(e.target.files[0])}
                        />
                    ...
                    <div className="form-group">
                        <button className="form-button" onClick={() => BlogEditSubmit()}>
                            Submit
                        </button>
                    ...
                    <TabPanel value={0}>
                        ...
                        <div className='px-5'>
                            ...
                            <div style={{ marginBottom: '20px' }}>
                                ...
                                />
                                <b >
                                    Note: Enter without Forward Slash '/'
                                </b>
                            </div>
                            <div className="form-group">
                                <label className="form-label">Description</label>
                                    ...
                                    <div className='px-5'>
                                        ...
                                        <div style={{ marginBottom: '20px' }}>
                                            ...
                                            <input
                                                type="text"
                                                value={blogEditSlug}
                                                onChange={(e) => setBlogEditSlug(e.target.value)}
                                                className="form-input"
                                            />
                                            <b >
                                                Note: Enter without Forward Slash '/'
